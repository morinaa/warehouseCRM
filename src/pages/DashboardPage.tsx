...
const DashboardPage = () => {
  const { data: me } = useQuery({ queryKey: ['me'], queryFn: api.getCurrentUser });
  ...
  const lowStock = products
    .filter((product) => product.stock && product.stock.minThreshold > 0 && product.stock.stockLevel < product.stock.minThreshold)
      ...
      <Stack spacing={6}>
        ...
        <SimpleGrid columns={{ base: 1, sm: 2, lg: 5 }} spacing={4}>
          ...
          <StatCard
            ...
            icon={FiPackage}
          />
          <StatCard label="Products" value={products.length} helper="Live SKUs" icon={FiBox} />
          <StatCard label="Avg cycle time" value={`${cycleTimeDays(orders)} days`} helper="Order to ship" icon={FiZap} />
          <StatCard label="Suppliers" value={suppliers.length} helper="Active vendors" icon={FiUsers} />
        </SimpleGrid>
  ...
  // Company (buyer/supplier) dashboard: scoped data only
  return (
    <Stack spacing={6}>
      <Heading size="lg">Company dashboard</Heading>
      <SimpleGrid columns={{ base: 1, sm: 2, lg: 3 }} spacing={4}>
        <StatCard label="My orders" value={orders.length} helper="Scoped to your company" icon={FiPackage} />
        <StatCard
          label="Open order value"
          value={`$${openOrderValue.toLocaleString()}`}
          helper="Pending, confirmed, shipped"
      ...
      <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={4}>
        <Card gridColumn={{ base: 'span 1', lg: 'span 1' }} borderWidth="1px" borderColor="gray.100">
          <CardBody>
            <Flex align="center" justify="space-between" mb={4}>
              <Heading size="md">Orders by status</Heading>
              <Badge colorScheme="brand" variant="subtle">
