...
const SupplierDetailPage = () => {
  ...
  // Ensure buyers always have at least their own company account in view
  const scopedAccounts = useMemo(() => {
    ...
    return [];
  }, [accounts, user?.companyId, user?.id]);
  ...
  useEffect(() => {
    ...
    const scoped = user?.companyId
      ? scopedAccounts.find((a) => a.id === user.companyId)?.id
      : undefined;
    const fallback = scoped ?? scopedAccounts[0]?.id ?? '';
    setAccountId(fallback);
  }, [scopedAccounts, user?.companyId, accountId]);
  ...
  const cart = useMemo(() => {
    const lines = supplierProducts
      .map((p) => ({
        ...
        unitPrice:
          p.tierPrices.find((t) => t.tierId === (scopedAccounts.find((a) => a.id === accountId)?.priceTierId ?? 'tier-standard'))
            ?.price ?? p.basePrice,
      }))
      .filter((l) => l.qty > 0);
  ...
  }, [supplierProducts, quantities, accounts, accountId]);

  const submitOrder = async () => {
    if (!accountId || scopedAccounts.length === 0) {
      const fallback = user?.companyId
        ? scopedAccounts.find((a) => a.id === user.companyId)
        : scopedAccounts[0];
      console.warn('Account missing, attempting fallback selection', {
        userId: user?.id,
    ...
    try {
      await createOrder.mutateAsync({
        ...
        paymentTerms: scopedAccounts.find((a) => a.id === accountId)?.paymentTerms ?? 'Net 30',
        warehouse: 'WH-BUYER',
  ...
  return (
    <Stack spacing={6}>
      <Flex justify="space-between" align="center" wrap="wrap" gap={3}>
        ...
        <Stack minW="260px">
          <Text fontWeight="semibold">Bill to (your company account)</Text>
          <Select value={accountId} isDisabled bg="white" maxW="280px">
            {scopedAccounts
              .filter((acct) => !user?.companyId || acct.id === user.companyId)
              .map((acct) => (
                <option key={acct.id} value={acct.id}>
                  {acct.name} ({acct.paymentTerms})
                </option>
              ))}
            {!accountId && <option value="">No account found</option>}
          </Select>
        </Stack>
      </Flex>
            ...
            {supplierProducts.map((product: Product) => {
              const unitPrice =
                product.tierPrices.find(
                  (t) => t.tierId === (scopedAccounts.find((a) => a.id === accountId)?.priceTierId ?? 'tier-standard'),
